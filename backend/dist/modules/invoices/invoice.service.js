...
class InvoiceService {
    ...
    async findById(id) {
        ...
        if (!invoice) {
            throw app_error_1.AppError.notFound('Invoice');
        }
        return invoice;
    }
    async generateInvoice(data) {
        ...
        if (contract.status !== 'active') {
            throw app_error_1.AppError.badRequest('Cannot generate invoice for inactive contract');
        }
        ...
        if (existingInvoice) {
            throw app_error_1.AppError.conflict('Invoice already exists for this month');
        }
        ...
        if (data.otherFees) {
            if (Array.isArray(data.otherFees)) {
                otherFeesTotal = data.otherFees.reduce((sum, fee) => sum + (fee.amount || 0), 0);
            }
            else if (typeof data.otherFees === 'object') {
                otherFeesTotal = Object.values(data.otherFees).reduce((sum, value) => sum + (Number(value) || 0), 0);
            }
        }
    ...
    async generateBatchInvoices(contractIds, invoiceMonth, dueDays) {
        if (!(invoiceMonth instanceof Date) || Number.isNaN(invoiceMonth.getTime())) {
            throw app_error_1.AppError.badRequest('invoiceMonth không hợp lệ');
        }
        if (!Array.isArray(contractIds)) {
            throw app_error_1.AppError.badRequest('contractIds phải là mảng');
        }
        ...
        if (contractsWithoutInvoice.length === 0) {
            throw app_error_1.AppError.badRequest('All selected contracts already have invoices for this month');
        }
        ...
        const invoices = await Promise.all(contractsWithoutInvoice.map(async (contract) => {
            // Simulate meter readings (in real app, these would come from actual readings)
            const electricityPrev = Math.floor(Math.random() * 100) + 100;
            const electricityCurr = electricityPrev + Math.floor(Math.random() * 200) + 50;
            const waterPrev = Math.floor(Math.random() * 10) + 5;
        ...
        return {
            ...
            skipped: contracts.length - invoices.length,
            invoices
        };
    }
    async update(id, data) {
        ...
        if (data.status === client_1.InvoiceStatus.paid && !invoice.paymentMethod && !data.paymentMethod) {
            throw app_error_1.AppError.badRequest('Payment method is required when marking as paid');
        }
        ...
        if (data.electricityPrice !== undefined || data.waterPrice !== undefined || data.otherFees !== undefined) {
            ...
            if (data.otherFees) {
                ...
                if (Array.isArray(invoice.otherFees)) {
                    otherFeesTotal = invoice.otherFees.reduce((sum, fee) => sum + (fee.amount || 0), 0);
                }
                else if (typeof invoice.otherFees === 'object') {
                    otherFeesTotal = Object.values(invoice.otherFees).reduce((sum, value) => sum + (Number(value) || 0), 0);
                }
            }
        ...
        const updatedInvoice = await prisma.invoice.update({
            ...
            data: {
                ...
                paidAt: data.status === client_1.InvoiceStatus.paid && !invoice.paidAt ? new Date() : undefined
            },
            include: {
                contract: {
                    include: {
    ...
    async processPayment(id, paymentMethod, paymentRef) {
        const invoice = await prisma.invoice.findUnique({
            where: { id }
        });
        if (!invoice) {
        ...
        if (invoice.status === client_1.InvoiceStatus.paid) {
            throw app_error_1.AppError.badRequest('Invoice is already paid');
        }
        const updatedInvoice = await prisma.invoice.update({
            where: { id },
            data: {
        ...
        const [total, paid, unpaid, overdue, totalAmount, paidAmount, unpaidAmount] = await Promise.all([
            ...
            prisma.invoice.aggregate({
                where: {
                    ...
                    status: { in: [client_1.InvoiceStatus.unpaid, client_1.InvoiceStatus.overdue] }
                },
                _sum: {
                    totalAmount: true
                }
    ...
    async getSummaryStats() {
        const [total, paid, unpaid, overdue, totalAmount, paidAmount, unpaidAmount] = await Promise.all([
            prisma.invoice.count(),
            prisma.invoice.count({ where: { status: client_1.InvoiceStatus.paid } }),
            prisma.invoice.count({ where: { status: client_1.InvoiceStatus.unpaid } }),
            prisma.invoice.count({ where: { status: client_1.InvoiceStatus.overdue } }),
            prisma.invoice.aggregate({
                _sum: { totalAmount: true }
            }),
            prisma.invoice.aggregate({
                where: { status: client_1.InvoiceStatus.paid },
                _sum: { totalAmount: true }
            }),
            prisma.invoice.aggregate({
                where: { status: { in: [client_1.InvoiceStatus.unpaid, client_1.InvoiceStatus.overdue] } },
                _sum: { totalAmount: true }
            })
        ]);
        return {
