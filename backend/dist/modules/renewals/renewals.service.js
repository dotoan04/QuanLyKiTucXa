...
class RenewalService {
    async checkRenewalEligibility(contractId) {
    ...
    async renewContract(data, userId) {
        const contract = await prisma.contract.findUnique({
        ...
        if (!student || contract.studentId !== student.id) {
            throw app_error_1.AppError.forbidden('You can only renew your own contract');
        }
        const eligibility = await this.checkRenewalEligibility(data.contractId);
        if (!eligibility.isEligible) {
            throw app_error_1.AppError.badRequest(eligibility.reason || 'Contract is not eligible for renewal');
        }
        if (data.newEndDate <= new Date()) {
            throw app_error_1.AppError.badRequest('New end date must be in the future');
        }
        if (contract.endDate && data.newEndDate <= contract.endDate) {
            throw app_error_1.AppError.badRequest('New end date must be after current end date');
        }
    ...
    async sendRenewalReminders() {
        ...
        const notifications = await Promise.all(expiringContracts.map(contract => prisma.notification.create({
            data: {
                userId: contract.student.userId,
                type: 'room_approved',
