...
class ReturnService {
  async createReturnRequest(data: ReturnRequestInput, userId: string) {
    ...
    if (contract.status !== 'active') {
      throw AppError.badRequest('Contract is not active')
    }
    ...
    if (!student || contract.studentId !== student.id) {
      throw AppError.forbidden('You can only return your own contract')
    }
    ...
    if (new Date(data.returnDate) < minReturnDate) {
      throw AppError.badRequest('Return date must be at least 7 days from now')
    }
    ...
    if (unpaidInvoices > 0) {
      throw AppError.badRequest(`You have ${unpaidInvoices} unpaid invoice(s). Please pay before returning room.`)
    }
    ...
    if (existingReturn) {
      throw AppError.conflict('Return request already exists')
    }
  ...
  async scheduleInspection(data: ScheduleInspectionInput) {
    ...
    if (!returnRequest) {
      throw AppError.notFound('Return request')
    }

    if (returnRequest.status !== 'pending') {
      throw AppError.badRequest('Return request is not in pending status')
    }
    ...
    if (!inspector) {
      throw AppError.notFound('Inspector')
    }
  ...
  async completeInspection(data: CompleteInspectionInput) {
    const returnRequest = await prisma.returnRequest.findUnique({
      where: { id: data.returnRequestId },
      include: {
        contract: {
    ...
    if (returnRequest.status !== 'scheduled') {
      throw AppError.badRequest('Return request is not scheduled')
    }
  ...
  async processRefund(data: ProcessRefundInput) {
    ...
    if (returnRequest.status !== 'inspected') {
      throw AppError.badRequest('Return request must be inspected first')
    }
  ...
  async getAllReturnRequests(params: { status?: string; page?: number; limit?: number }) {
  ...
  async cancelReturnRequest(id: string, userId: string) {
    ...
    if (!student || returnRequest.contract.studentId !== student.id) {
      throw AppError.forbidden('You can only cancel your own return request')
    }

    if (returnRequest.status !== 'pending') {
      throw AppError.badRequest('Can only cancel pending return requests')
    }
